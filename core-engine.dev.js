...
var MCPE_VERSION_SUPPORT = {17: true};
...
var nonSavesObjectSaver = Saver.registerObjectSaver("nonSavesObjectSaver", {read: function () {
    return null;
}, save: function () {
    return null;
}});
var GuiUtils = {Run: function (func) {
    MCSystem.runAsUi({run: function () {
        try {
            func();
        }
        catch (e) {
...
var FileTools = {mntdir: "/mnt", root: android.os.Environment.getExternalStorageDirectory().getAbsolutePath() + "/", moddir: __packdir__ + "innercore/mods/", mkdir: function (dir) {
...
var Threading = {threads: [], formatFatalErrorMessage: function (error, name, priority, formatFunc) {
...
var ModAPI = {modAPIs: {}, registerAPI: function (name, api, descr) {
    if (!descr) {
        descr = {};
    }
    if (!descr.name) {
...
var SaverAPI = {addSavesScope: function (name, loadFunc, saveFunc) {
...
isItemSpendingAllowed: function() {
...
var GameObjectRegistry = {gameObjectTypes: {}, activeGameObjects: {}, genUniqueName: function (name) {
    name += "";
    ...
    gameObjectClass.saverId = Saver.registerObjectSaver(gameObjectClass.gameobjectName, {read: function (obj) {
...
var TileEntityBasePrototype = {remove: false, isLoaded: false, __initialized: false, defaultValues: {}, update: function () {
...
var TileEntity = {tileEntityPrototypes: {}, tileEntityList: [], resetEngine: function () {
    this.tileEntityList = [];
    ...
    Prototype.saverId = Saver.registerObjectSaver(saverName, {read: function (obj) {
        var instance = {};
        ...
        return {data: obj.data, container: obj.container, liquidStorage: obj.liquidStorage, coords: {x: obj.x, y: obj.y, z: obj.z, d: obj.dimension}};
    }});
    ...
    return true;
}, destroyTileEntityAtCoords: function (x, y, z) {
    ...
    var checker = {tileEntity: tileEntity, age: 0, update: function () {
...
var WorldGeneration = {generatorUpdatable: null, checkTile: function (x, z) {
    ...
    this.generatorUpdatable = {age: 0, delay: 3, surface_radius: 3, underground_radius: 1, thread_optimization: false, generation_priority: 0, ticking_priority: 0, debug: false, debug_max_time: 0, update: function () {
...
var WorldGenerationUtils = {isTerrainBlock: function (id) {
    ...
    return {x: parseInt((Math.random() + cx) * 16), z: parseInt((Math.random() + cz) * 16)};
    ...
    return {x: x, y: GenerationUtils.findSurface(x, y, z), z: z};
}, findHighSurface: function (x, z) {
    return this.findSurface(x, 128, z);
}, findLowSurface: function (x, z) {
    return this.findSurface(x, 64, z);
}, __lockedReal: {id: 0, data: 0}, lockInBlock: function (id, data, checkerTile, checkerMode) {
...
var BLOCK_BASE_PROTOTYPE = {__validBlockTypes: {createBlock: true, createBlockWithRotation: true}, __define: function (item) {
    ...
    if (shape.length >= 6) {
        BlockRegistry.setBlockShape(this.id, {x: shape[0], y: shape[1], z: shape[2]}, {x: shape[3], y: shape[4], z: shape[5]});
    } else {
    ...
    return null;
}, getDrop: null, onPlaced: null, onItemUsed: null};
var BlockRegistry = {idSource: BlockID, dropFunctions: {}, popResourceFunctions: {}, getNumericId: function (id) {
    if (typeof (id) == "string") {
    ...
    for (var i in defineData) {
        ...
        for (var j in rotated) {
            rotatedBlocks.push({name: block.name, texture: rotated[j], inCreative: block.inCreative && j == 0});
        }
    }
    ...
    this.registerDropFunctionForID(id, function (blockCoords, blockID, blockData, diggingLevel) {
        ...
        if (diggingLevel >= level) {
            return [[BlockRegistry.convertBlockToItemId(blockID), 1, resetData? 0: blockData]];
        }
    }, level);
}, setDestroyLevel: function (namedID, level) {
    var numericID = this.getNumericId(namedID);
    if (numericID == -1) {
        return false;
    }
...
}, onBlockDestroyed: function (coords, fullTile, byHand) {
    ...
    if (result != null) {
        ...
        if (GameAPI.isItemSpendingAllowed() || !byHand) {
            for (var i in result) {
...
}, onBlockPoppedResources: function(coords, block, f, i) {
    var func = this.popResourceFunctions[block.id];
    if (func) {
        func(coords, block, f, i);
    }
}, getBlockDropViaItem: function (block, item, coords) {
...
}, placeFuncs: [], registerPlaceFunctionForID: function (block, func) {
...
}, TYPE_BASE: "createBlock", TYPE_ROTATION: "createBlockWithRotation", setPrototype: function (namedID, Prototype) {
...
var ITEM_BASE_PROTOTYPE = {__validItemTypes: {createItem: true, createFoodItem: true, createArmorItem: true, createThrowableItem: true}, __define: function (item) {
...
var ItemRegistry = {idSource: ItemID, useFunctions: {}, throwableFunctions: {}, getNumericId: function (id) {
    if (typeof (id) == "string") {
        var _id = this.idSource[id];
        if (!_id) {
            Logger.Log("Invalid item namedID: " + id + ", -1 will be returned", "ERROR");
    ...
    var validArmorTypes = {helmet: {id: 0}, chestplate: {id: 1}, leggings: {id: 2}, boots: {id: 3}};
    if (!params) {
        params = {};
    }
...
}, getName: function (id, data, encode) {
...
}, onItemUsed: function (coords, item, block) {
    this.__func = this.useFunctions[item.id];
    if (this.__func) {
        var result = this.__func(coords, item, block);
    }
...
}, iconOverrideFunctions: {}, registerIconOverrideFunction: function (namedID, func) {
    var numericID = this.getNumericId(namedID);
    if (numericID == -1) {
        return false;
    }
...
}, noTargetUseFunctions: {}, registerNoTargetUseFunction: function (namedID, func) {
    var numericID = this.getNumericId(namedID);
    if (numericID == -1) {
        return false;
    }
...
}, usingReleasedFunctions: {}, registerUsingReleasedFunction: function (namedID, func) {
    var numericID = this.getNumericId(namedID);
    if (numericID == -1) {
        return false;
    }
    this.usingReleasedFunctions[numericID] = func;
}, onUsingReleased: function (item, ticks) {
    var func = this.usingReleasedFunctions[item.id];
    if (func) {
        func(item, ticks);
    }
}, usingCompleteFunctions: {}, registerUsingCompleteFunction: function (namedID, func) {
    var numericID = this.getNumericId(namedID);
    if (numericID == -1) {
        return false;
    }
    this.usingCompleteFunctions[numericID] = func;
}, onUsingComplete: function (item) {
    var func = this.usingCompleteFunctions[item.id];
    if (func) {
        func(item);
    }
}, dispenseFunctions: {}, registerDispenseFunction: function (namedID, func) {
    var numericID = this.getNumericId(namedID);
    if (numericID == -1) {
        return false;
    }
    this.dispenseFunctions[numericID] = func;
}, onDispense: function (coords, item) {
    var func = this.dispenseFunctions[item.id];
    if (func) {
        func(coords, item);
    }
}, TYPE_BASE: "createItem", TYPE_FOOD: "createFoodItem", TYPE_ARMOR: "createArmorItem", TYPE_THROWABLE: "createThrowableItem", setPrototype: function (namedID, Prototype) {
...
var ArmorRegistry = {registerFuncs: function (id, funcs) {
...
var ToolAPI = {blockMaterials: {}, toolMaterials: {}, toolData: {}, blockData: {}, needDamagableItemFix: false, addBlockMaterial: function (name, breakingMultiplier) {
    ...
    var resultDestroyTime = toolData.calcDestroyTime(toolItem, coords, fullBlock, {base: baseDestroyTime, devider: devider, modifier: modifier}, baseDestroyTime / devider / modifier, enchantData);
...
}, destroyBlockHook: function (coords, block, item) {
...
}, LastAttackTime: 0, playerAttackHook: function (attacker, victim, item) {
	var toolData = this.getToolData(item.id);
	var enchant = this.getEnchantExtraData(item.extra);
...
}, dropOreExp: function (coords, minVal, maxVal, modifier) {
    this.dropExpOrbs(coords.x + 0.5, coords.y + 0.5, coords.z + 0.5, minVal + parseInt(Math.random() * (maxVal - minVal + 1 + modifier)));
...
BlockRegistry.registerDropFunctionForID(454, function(coords, id, data, level){ // lit smoker
	if (level >= 1){
		return [[BlockRegistry.convertBlockToItemId(453), 1, 0]];
	}
	return [];
}, 1);
...
BlockRegistry.registerDropFunctionForID(469, function(coords, id, data, level){ // lit blast furnace
	if (level >= 1){
		return [[BlockRegistry.convertBlockToItemId(451), 1, 0]];
	}
	return [];
}, 1);
...
var LiquidRegistry = {liquidStorageSaverId: Saver.registerObjectSaver("_liquidStorage", {read: function (obj) {
...
}}), liquids: {}, registerLiquid: function (key, name, uiTextures, modelTextures) {
    if (this.liquids[key]) {
        Logger.Log("liquid key " + key + " is not unique, new liquid will replace old one", "WARNING");
    }
    this.liquids[key] = {key: key, name: name || key, uiTextures: uiTextures || [], uiCache: {}, modelTextures: modelTextures || [], addUITexture: function (name) {
...
}, FullByEmpty: {}, EmptyByFull: {}, registerItem: function (liquid, empty, full) {
...
var WorldAPI = {isLoaded: false, setLoaded: function (isLoaded) {
...
registerBlockChangeCallback: function(ids, callback) {
...
}, onBlockChanged: function(coords, block1, block2, int1, int2) {
...
}, __inworld: {nativeSetBlock: function (x, y, z, id, data) {
...
}, setBlock: NativeAPI_setTile, setFullBlock: function (x, y, z, fullTile) {
    NativeAPI_setTile(x, y, z, fullTile.id, fullTile.data);
}, getBlock: function (x, y, z) {
    var tile = NativeAPI_getTileAndData(x, y, z);
    return {id: ((tile >> 24 == 1)? -1: 1) *(tile & 0xFFFF), data: ((tile >> 16) & 0xFF)};
}, getBlockID: NativeAPI_getTile, getBlockData: NativeAPI_getData, destroyBlock: function (x, y, z, drop) {
    var tile = this.getBlock(x, y, z);
    if (drop) {
        BlockRegistry.onBlockDestroyed({x: x, y: y, z: z}, tile, false);
    }
    Level.destroyBlock(x, y, z, drop);
}, getLightLevel: requireMethodFromNativeAPI("api.NativeAPI", "getBrightness"), isChunkLoaded: function (x, z) {
    ...
    return {rain: Level.getRainLevel(), thunder: Level.getLightningLevel()};
    ...
    return {r: (color >> 16) & 255, g: (color >> 8) & 255, b: (color >> 0) & 255};
...
}}, __inmenu: {nativeSetBlock: function () {
...
var AnimatorToken = {__current: 1, genToken: function () {
...
function Texture(path) {
    ...
    this.getResolution = function () {
        return {w: this.resolution.w * this.pixelScale, h: this.resolution.h * this.pixelScale};
    };
    this.pixelScale = 1;
...
var EntityRenderGlobalCache = {globalCache: {}, saveRenderAPI: function (api, name, isLocal) {
    var cache;
    if (isLocal) {
...
function RenderAPI(params) {
    ...
    this._setPartRecursive = function (part, data, coords) {
        for (var i in data) {
            ...
            if (element.children) {
                this._setPartRecursive(part, elementCoords, element.children);
            }
        }
    };
    this.localCache = {};
    ...
    this.toCache = function () {
        return {renderer: this.renderer, renderId: this.renderId, model: this.model, parts: this.parts, isChangeable: this.isChangeable};
    };
...
ce_default_entity_render.setPart("body", [{type: "box", coords: {x: 0, y: 16, z: 0}, uv: {x: 0, y: 0}, size: {x: 16, y: 16, z: 16}}], {});
...
var EntityAIIdle = new EntityAI({getDefaultPrioriy: function () {
    return 1;
...
var EntityAIFollow = new EntityAI({data: {target: null, targetEntity: null, movingYaw: 0}, params: {speed: 0.2, jumpVel: 0.45, rotateSpeed: 0.4, rotateRatio: 0.5, rotateHead: true, denyY: true}, setParams: function (params) {
    for (var name in params) {
        this.params[name] = params[name];
    }
}, execute: function () {
...
var EntityAIPanic = new EntityAI({getDefaultPriority: function () {
    return 3;
}, getDefaultName: function () {
    return "panic";
}, params: {speed: 0.22, angular_speed: 0.5}, data: {yaw: 0, add: 0}, setParams: function (params) {
    for (var name in params) {
        this.params[name] = params[name];
    }
}, randomize: function () {
    ...
    EntityAPI.moveToLook(this.entity, {speed: this.params.speed, denyY: true, jumpVel: 0.45});
}});
var EntityAIWander = new EntityAI({getDefaultPriority: function () {
    return 2;
}, getDefaultName: function () {
    return "wander";
}, params: {speed: 0.08, angular_speed: 0.1, delay_weight: 0.3}, data: {yaw: 0, add: 0, delay: false, _delay: true}, setParams: function (params) {
    for (var name in params) {
        this.params[name] = params[name];
    }
}, randomize: function () {
    ...
    if (!this.data.delay) {
        this.data.yaw += this.data.add;
        EntityAPI.moveToLook(this.entity, {speed: this.params.speed, denyY: true, jumpVel: this.data._delay ? 0 : 0.45});
    }
    this.data._delay = this.data.delay;
}});
var EntityAIAttack = new EntityAI({params: {attack_damage: 5, attack_range: 2.5, attack_rate: 12}, data: {timer: 0, target: null}, execute: function () {
    if (this.data.target) {
...
var EntityAISwim = new EntityAI({getDefaultPriority: function () {
    return -1;
}, getDefaultName: function () {
    return "swim";
...
var EntityAIPanicWatcher = new EntityAIWatcher({params: {panic_time: 200, priority_panic: 5, priority_default: 1, name: "panic"}, data: {timer: -1}, hurtBy: function () {
...
var EntityAIController = {currentPriority: 0, loadedAI: {}, loadedData: {}, isAILoaded: false, getAITypes: function () {
    return {"main": {type: EntityAIIdle}};
    ...
    for (var name in types) {
        ...
        this.loadedAI[name] = {AI: AI, priority: data.priority || AI.getDefaultPriority(), enabled: enabled};
        if (enabled) {
            AI.executionStarted();
        }
    }
    ...
    for (var name in this.loadedAI) {
        var ai = this.loadedAI[name];
        data[name] = {e: ai.enabled, p: ai.priority, d: ai.AI.data};
    }
    return data;
}, read: function (data) {
    this.loadedData = data;
}};
var EntityDescriptionController = {isDynamic: false, getHitbox: function () {
...
var EntityVisualController = {modelWatchers: {}, modelWatcherStack: [], getModels: function () {
    return {"main": ce_default_entity_model};
    ...
    if (ticks >= 0) {
        this.modelWatcherStack.unshift({name: name, ticks: ticks});
    } else {
        this.modelWatcherStack = [{name: name, ticks: -1}];
    }
...
var EntityEventController = {update: function () {
    this.tick();
}, tick: function () {
}, removed: function () {
...
var CustomEntityConfig = {unloaded_despawn_time_in_secs: 600, despawn_unloaded_entities: true};
...
function CustomEntity(nameId) {
    ...
    this.__base_type = 28;
    var self = this;
    this.saverId = Saver.registerObjectSaver(this.nameId, {read: function (obj) {
        self.read(obj);
        return null;
    }, save: function (obj) {
        return obj.save();
    ...
    this.save = function () {
        var data = {entity: parseInt(this.entity), age: this.age, oneAndHalf: 1.5, unloaded: this.unloadedTime, controllers: {}, rp: this.realPosition};
        for (var name in this.controllers) {
...
var MobRegistry = {customEntities: {}, loadedEntities: [], registerEntity: function (name) {
...
var EntitySpawnRegistry = {spawnData: [], registerSpawn: function (entityType, rarity, condition, denyNaturalDespawn) {
    if (!condition) {
        condition = function () {
            return parseInt(Math.random() * 3 + 1);
        };
    }
    this.spawnData.push({type: entityType, rarity: rarity, condition: condition, denyNaturalDespawn: denyNaturalDespawn});
    ...
    for (var i = 0; i < this.spawnData.length; i++) {
        var position = {x: (x + Math.random()) * 16, z: (z + Math.random()) * 16};
...
var PlayerAPI = {get: function () {
    ...
    return {get: this.getExperience, set: this.setExperience, add: this.addExperience};
    ...
    return {get: this.getLevel, set: this.setLevel, add: this.addLevel};
}, getLevel: function () {
    ...
    return {set: this.setFlying, get: this.getFlying, getEnabled: this.getFlyingEnabled, setEnabled: this.setFlyingEnabled};
...
function AnimationItem(x, y, z) {
    ...
    this.setItemSize = function(size) {
        if (this.__postedItem) {
            this.__postedItem.size = [x, y, z];
        }
        if (this.lastItemModel) {
            this.lastItemModel.setScale(size);
        }
    }
...
var EntityAPI = {getAll: function () {
    ...
    return {pitch: __degreesToRad(-Entity.getPitch(ent)), yaw: __degreesToRad(Entity.getYaw(ent))};
    ...
    return {x: -Math.sin(angle.yaw) * Math.cos(angle.pitch), y: Math.sin(angle.pitch), z: Math.cos(angle.yaw) * Math.cos(angle.pitch)};
    ...
    var pos = this.getPosition(ent);
    var target = {x: pos.x + vec.x * speed, y: pos.y + vec.y * speed, z: pos.z + vec.z * speed};
    ...
    return {pitch: Math.asin(vec.y) || 0, yaw: Math.atan2(-vec.x, vec.z) || 0};
...
var CONSTANT_REPLACEABLE_TILES = {0: true, 8: true, 9: true, 10: true, 11: true, 31: true, 51: true, 78: true, 106: true};
...
Callback.addCallback("ItemUse", function (coords, item, block) {
    ...
    if (TileEntity.isTileEntityBlock(block.id)) {
        var tileEntity = TileEntity.getTileEntity(coords.x, coords.y, coords.z);
        if (!tileEntity) {
            tileEntity = TileEntity.addTileEntity(coords.x, coords.y, coords.z);
        }
        ...
        if (TileEntity.isTileEntityBlock(item.id)) {
            ...
            if (canTileBeReplaced(tile)) {
                if (placeFunc) {
                    ...
                    var tileEntity = TileEntity.addTileEntity(placeCoords.x, placeCoords.y, placeCoords.z);
                } else {
                    setTile(coords.relative.x, coords.relative.y, coords.relative.z, item.id);
                    var tileEntity = TileEntity.addTileEntity(coords.relative.x, coords.relative.y, coords.relative.z);
                }
                if (GameAPI.isItemSpendingAllowed()) {
                    setPlayerItem(item.id, item.count - 1, item.data);
                }
...
var EntityDataRegistry = {isLevelLoaded: false, entityData: {}, entityDataTyped: {}, getAllData: function () {
...
var ParticleAnimator = {addParticle: Particle_addParticle, addFarParticle: Particle_addFarParticle, line: function (particle, coords1, coords2, gap, vel, data) {
    gap = gap || 1;
    var delta = {x: coords2.x - coords1.x, y: coords2.y - coords1.y, z: coords2.z - coords1.z};
    vel = vel || {x: 0, y: 0, z: 0};
    delta.size = Math.sqrt(delta.x * delta.x + delta.y * delta.y + delta.z * delta.z);
    delta.x /= delta.size;
    delta.y /= delta.size;
    ...
    newNoiseLayer: function(descr) {
        if (descr.octaves) {
            ...
            if (Array.isArray(descr.octaves)) {
                ...
                var scale = this.getVec3(descr.octaves.scale, {x: default_scale, y: default_scale, z: default_scale});
                var mul = 2 * ((1 << count) - 1) / (1 << count);
                for (var i = 0; i < count; i++) {
                    layer.addOctave(this.newNoiseOctave({scale: {x: 1 / scale.x, y: 1 / scale.y, z: 1 / scale.z}, weight: weight / mul, seed: seed + i}));
...
Translation.addTranslation("system.thread_stopped", {en: "All mods are stopped because of\nfatal error on main thread.\nPlease re-enter the world.", ru: "Работа модов приостановлена из-за ошибки,\nперезайдите в мир."});

var CoreAPI = {getCoreAPILevel: function () {
...
}}, Resources: Resources, Dimensions: Dimensions, CustomBiome: CustomBiome, Commands: Commands, FileTools: FileTools, Logger: Logger, Translation: Translation, Threading: Threading, Config: Config, UI: UI, UpdatableAPI: Updatable, Updatable: Updatable, TileEntity: TileEntity, MobRegistry: MobRegistry, MobSpawnRegistry: EntitySpawnRegistry, Callback: Callback, GameObject: GameObject, GameObjectRegistry: GameObjectRegistry, ModAPI: ModAPI, Saver: SaverAPI, World: WorldAPI, Entity: EntityAPI, AddonEntityRegistry: AddonEntityRegistry, Player: PlayerAPI, Game: GameAPI, Render: RenderAPI, Texture: Texture, EntityModel: ModelAPI, EntityModelWatcher: ModelWatcher, EntityAIClass: EntityAI, EntityAIWatcher: EntityAIWatcher, EntityAI: {Idle: EntityAIIdle, Follow: EntityAIFollow, Panic: EntityAIPanic, Wander: EntityAIWander, Attack: EntityAIAttack, Swim: EntityAISwim, PanicWatcher: EntityAIPanicWatcher}, GenerationUtils: WorldGenerationUtils, Animation: {base: AnimationBase, Base: AnimationBase, item: AnimationItem, Item: AnimationItem}, Particles: ParticleAnimator, IDRegistry: IDRegistry, IDData: {item: ItemID, block: BlockID}, ItemID: ItemID, BlockID: BlockID, Block: BlockRegistry, BlockRenderer: BlockRenderer, ICRender: ICRender, Item: ItemRegistry, Recipes: Recipes, ToolAPI: ToolAPI, Armor: ArmorRegistry, LiquidRegistry: LiquidRegistry, Native: {ArmorType: ArmorType, ItemCategory: ItemCategory, ParticleType: ParticleType, Color: ChatColor, EntityType: EntityType, MobRenderType: EntityRenderType, PotionEffect: MobEffect, Dimension: DimensionId, ItemAnimation: UseAnimation, BlockSide: BlockFace, Enchantment: Enchantment, EnchantType: EnchantType, BlockRenderLayer: BlockRenderLayer, GameMode: GameMode, GameDifficulty: GameDifficulty}, alert: print};
